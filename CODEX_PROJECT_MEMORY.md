...
- 舵机单路测试配置（2026-08-06，待实机验证）：选用扩展板 S2=PB9，复用为 TIMA0_CCP1；SysConfig 固定仅启用 CCP1，定时器时钟 1 MHz、周期 20000 tick（50 Hz），未占用 S1=PB8、PA0 或互补输出引脚。新增 BSP/servo.c/.h，提供 Servo_Init() 与 Servo_SetAngle()，按 500 us~2500 us 映射 0°~180°；empty.c 舵机测试逻辑已移除，当前主循环为空。Keil 工程已加入 BSP/servo.c。舵机须使用独立稳定的 5 V 供电，并与 MSP 共地。
